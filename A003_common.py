...
class EnsembledRidgeCV(object):
    ...
    #marjan
    def get_neme(self):
        return "EnsembledRidgeCV"


    def predict(self, x, return_std=False, return_member_predictions=False):
        #print('A003_common.EnsembledRidgeCV.predict method')
        # print("self.model_ensemble: ")
        # print(type(self.model_ensemble), len(self.model_ensemble))
        # print(type(self.model_ensemble[0]), len(self.model_ensemble[0]))

        yhats = [m['model'].predict(x[:, m['feature_idx']]) for m in self.model_ensemble]
        # print("yhats in prediction method: " , type(yhats), len(yhats), len(yhats[0]))
        yhat_mat = np.stack(yhats) # [n_members x x.shape[0]]
        # print("yhat_mat in prediction method: " , type(yhat_mat), len(yhat_mat), len(yhat_mat[0]))
        
        #marjan: calculate the mean on columns 
        yhat_mu = np.mean(yhat_mat, axis=0)
        #marjan: calculate the std on columns 
        yhat_std = np.std(yhat_mat, axis=0)
        
        
        to_return = (yhat_mu,self.model_ensemble)
...
def train_ensembled_ridge(x_train, y_train, n_members=100, subspace_proportion=1.0, pval_cutoff=0.01, 
    ...
    #print("A003_common.train mehtod for ", model.get_neme)
...
def cv_train_ridge_with_sparse_refit(x_train, y_train, pval_cutoff=0.01, do_sparse_refit=False, normalize=True):
    ...
    #print("==A003.cv.trian.ridge.sparse.refit")
    #model = RidgeCV(alphas=np.logspace(-6,6,1000), gcv_mode='auto', normalize=normalize, store_cv_values=True)
    #what is below parameter? 
    ######normalize
    model = RidgeCV(alphas=np.logspace(-6,6,1000), gcv_mode='auto', store_cv_values=True)
    #print("A003_common.train mehtod for RidgeCV")
    ...
    if do_sparse_refit:
        ...
        #what is below parameter? 
        ######normalize
        model_sparse = Ridge(alpha=model.alphas[sparse_alpha_idx])
...
def train_blr(x_train, y_train, normalize=True):
    ...
    print("A003_common.train mehtod for BayesianRidge")
...
def cv_train_knn(x_train, y_train, pval_cutoff=0.001, do_sparse_refit=False, normalize=True):
    ...
    print("A003_common.train mehtod for KNeighborsRegressor")
...
def cv_train_GP(x_train, y_train, pval_cutoff=0.001, kernel=None):
    ...
    print("A003_common.train mehtod for GaussianProcessRegressor")
...
def cv_train_lasso_lars_with_sparse_refit(x_train, y_train, pval_cutoff=0.001, do_sparse_refit=True):
    ...
    print("A003_common.train mehtod for LassoLarsCV")
    model.fit(x_train, y_train)
...
def generate_simplified_and_fused_gen_set(sn_df, fp_df):
    ...
    print("sn_df: " + str(shape(sn_df)))
    print("fp_df: " + str(shape(fp_df)))
...
def generate_fp_homologs_parents_df():
    ...
    parent_df_file = '/content/drive/MyDrive/Colab Notebooks/Marjan/Low N/analysis/A003_policy_optimization/intermediate_files/20190401_selected_aequorea_FPs_for_synthesis_with_well_info.csv'
...
def generate_count_summary_df_for_exp9():
    ...
    exp9_count_df = pd.read_csv('/content/drive/MyDrive/Colab Notebooks/Marjan/Low N/analysis/A003_policy_optimization/intermediate_files/Exp9_aa_seq_count_table.csv')
